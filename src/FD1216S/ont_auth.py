...
# 使用LOID认证ONU        
def auth_by_loid(tn, PonID, OnuID, Ont_Lineprofile_ID, Ont_Srvprofile_ID,ONU_MAC,Epon_LOID):
    ...
    command_write = 'ont add %s %s loid-auth %s ont-lineprofile-id %s ont-srvprofile-id %s' \
                    % (PonID, OnuID, Epon_LOID, Ont_Lineprofile_ID, Ont_Srvprofile_ID)
    tn.write(command_write.encode('ascii') + b"\n")
    tn.read_until(b"OLT(config-interface-epon-0/0)#", timeout=2)
    time.sleep(10)
    # 判断OLT上，ONU是否添加成功
...
def auth_by_loid_password(tn, PonID, OnuID, Ont_Lineprofile_ID, Ont_Srvprofile_ID,ONU_MAC,Epon_LOID, Epon_LOID_PASSWORD, ):
    ...
    command_write = 'ont add %s %s loid-auth %s password-auth %s ont-lineprofile-id %s ont-srvprofile-id %s' \
                    % (PonID, OnuID, Epon_LOID, Epon_LOID_PASSWORD, Ont_Lineprofile_ID, Ont_Srvprofile_ID)
    tn.write(command_write.encode('ascii') + b"\n")
    tn.read_until(b"OLT(config-interface-epon-0/0)#", timeout=2)
    time.sleep(10)
    # 判断OLT上，ONU是否添加成功
