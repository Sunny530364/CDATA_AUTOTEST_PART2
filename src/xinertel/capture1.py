...
def create_stream(port, name, rate=10, stream_header=['Ethernet.ethernetII', 'VLAN.vlan', 'IPv4.ipv4', 'UDP.udp'],
                  packet_length=1024):
    '''
...
def rate_test(port_location, down_stream_header, up_stream_header, rate=10, duration=10,
    ...
    cdata_info('port1的端口状态：%s ;port2的端口状态：%s' % (port1.get_children('EthCopper')[0]._LinkStatus._name_,
...
def unicast_test(port_location, down_stream_header, up_stream_header, num=1, dataclassname=StreamBlockStats,
    ...
    if dataclassname == StreamBlockStats:
        for i in range(5):
            result_stats = []
            print('第%d次判断端口状态:port1的端口状态：%s ;port2的端口状态：%s'%((i+1), port1.get_children('EthCopper')[0]._LinkStatus._name_,
                                                 port2.get_children('EthCopper')[0]._LinkStatus._name_))
            if port2.get_children('EthCopper')[0]._LinkStatus._name_ == 'UP' and port1.get_children('EthCopper')[
                ...
                #                 FilterMode=EnumFilterMode.NONE, BufferFullAction=EnumBufferFullAction.WRAP,
                #                 StartingFrameIndex=10, AttemptDownloadPacketCount=1000)
                ...
                print('frist',cap_conf_1.__dict__)
                print('frist', cap_conf_2.__dict__)
                ...
                # #把报文导出
                # download_cap_data_cmd = DownloadCaptureDataCommand(CaptureConfigs=cap_conf_1.handle, FileDir='E:/Temp',
                #                                                    FileName='test.pcap', MaxDownloadDataCount=50)
                #
                # download_cap_data_cmd.execute()
                #
                # abort_download_data_cmd = AbortDownloadDataCommand(CaptureConfigs=cap_conf_1.handle)
                # abort_download_data_cmd.execute()
                # print('abort', cap_conf_1.__dict__)
        ...
        time.sleep(2)
        # stop_cap_cmd = StopCaptureCommand(CaptureConfigs=cap_conf_1.handle)
        # stop_cap_cmd.execute()
        print('end', cap_conf_1.__dict__)
        print('end', cap_conf_2.__dict__)
        # #把报文导出
        download_cap_data_cmd = DownloadCaptureDataCommand(CaptureConfigs=cap_conf_1.handle, FileDir='E:/Temp',
                                                           FileName='test.pcap', MaxDownloadDataCount=50)
...
if __name__ == '__main__':
    ...
    #                          rate=10,
    #                          duration=duration)
    result_stats=unicast_test(port_location, down_stream_header, up_stream_header, num=1, dataclassname=StreamBlockStats,
                 stream_header=['Ethernet.ethernetII', 'VLAN.vlan', 'IPv4.ipv4', 'UDP.udp'], duration=10)
    check_stream_static(result_stats[0], result_stats[1])
