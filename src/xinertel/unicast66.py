...
def captures_filename():
    ...
    packet_folder_path = os.path.join(current_path, "packets")
...
def unicast_test(port_location,down_stream_header,up_stream_header,num=1,dataclassname=StreamBlockStats,stream_header=['Ethernet.ethernetII', 'VLAN.vlan', 'IPv4.ipv4','UDP.udp'],duration=10):
    ...
    if dataclassname==StreamBlockStats:
        ...
        for i in range(5):
            cdata_info('第%d次查看状态：port1的端口状态：%s ;port2的端口状态：%s' % ((i + 1), port1.get_children('EthCopper')[0]._LinkStatus._name_,port2.get_children('EthCopper')[0]._LinkStatus._name_))
            result_stats = []
            if port2.get_children('EthCopper')[0]._LinkStatus._name_ == 'UP' and port1.get_children('EthCopper')[
                0]._LinkStatus._name_ == 'UP':
                # create result view
