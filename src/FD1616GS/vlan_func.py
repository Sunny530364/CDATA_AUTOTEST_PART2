...
def ont_port_transparent(tn,Ont_Srvprofile_ID):
    '''
...
def ont_port_trunk(tn,Ont_Srvprofile_ID,Ont_Port_ID,Vlan_list):
    # 进入服务模板视图下
    tn.write('ont-srvprofile gpon profile-id {} '.format(Ont_Srvprofile_ID).encode() + b'\n')
    result = tn.read_until('OLT(config-ont-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    if result:
...
def ont_port_vlan_del(tn,Ont_Srvprofile_ID,Ont_Port_ID,Vlan_list):
    # 进入服务模板视图下
    tn.write('ont-srvprofile gpon profile-id {} '.format(Ont_Srvprofile_ID).encode() + b'\n')
    result = tn.read_until('OLT(config-ont-srvprofile-{})#  '.format(Ont_Srvprofile_ID).encode(), timeout=2)
    if result:
...
def ont_port_translate(tn,Ont_Srvprofile_ID,Ont_Port_ID,S_Vlan_list,C_Vlan_list):
    '''
