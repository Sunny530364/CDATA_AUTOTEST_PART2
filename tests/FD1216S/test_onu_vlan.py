...
def test_onu_transparent(login):
    ...
    步骤3:配置onu的端口vlan模式为transparent。
    步骤4:发流测试。
...
def test_onu_tag(onu_vlan_suit):
    ...
    with allure.step('步骤3:配置onu的端口vlan模式为tag 2000。'):
        assert  ont_port_tag_profile(tn,Ont_Srvprofile_ID,User_Vlan='2000')
    with allure.step("步骤4:打流测试(上行不带tag流通)"):
        ...
        up_stream_header = (
            'ethernetII_1.sourceMacAdd=00:00:00:22:22:21   ethernetII_1.destMacAdd=00:00:00:11:11:11 ipv4_1.source=192.168.1.12 ipv4_1.destination=92.168.1.21',
        )
        result_stats = unicast_test(port_location=port_location, down_stream_header=down_stream_header,
                               up_stream_header=up_stream_header, num=1,
...
def test_onu_translate(onu_vlan_suit):
    ...
    # with allure.step('步骤1:配置onu的端口vlan模式为transparent '):
    #     ont_port_transparent_profile(tn)
...
if __name__ == '__main__':
    pytest.main(['-s','-x','test_onu_vlan.py::test_onu_transparent'])
